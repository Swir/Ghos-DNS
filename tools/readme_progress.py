...
def main() -> int:
    ...
    if args.check:
        ...
        icon = OUT / "app-icon.svg"
        hero = OUT / "hero.svg"
        for path in (template, icon, hero):
            if not path.exists():
                raise SystemExit(f"missing {path.relative_to(ROOT)}")
            _validate(path.read_text(encoding="utf-8"))
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        for required in ("assets/readme/progress-card.svg", "assets/readme/progress-mini.svg", "Product progress | **N/A**", "## 🔎 Search Keywords"):
